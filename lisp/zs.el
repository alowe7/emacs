...
(let ((map (symbol-function  'ctl-\-prefix))) ;; do redefine keys, tho.
  ...
  (define-key map "" 'push-scope)
  (define-key map "" 'pop-scope)
  (define-key map "" 'swap-scope)
  (define-key map "" 'roll-scope)
  (define-key map "" 'prune-cscope-search)
  (define-key map "
" 'pop-to-cscope)
  (define-key map "" 'stopscope)
  ;; if you like function keys, do this:
  ;;		       (cscope-keys)
  map
  )
...
(defun cscope-modify-mode-line ()
  (interactive)
  (let* ((a (intern (buffer-name)))
				 (v (qsaved-cscope-output))
				 (i (max 1 (qsaved-cscope-output-index)))

				 (len (length v))
				 (j (1- i))
				 (x (nth j v)))
	  (cscope-format-mode-line (- len j) len (qsave-cscope-cell-query x))
	  )
	)

(defun cscope-toggle-tpath ()
  "toggle usage of tpath in finding files"
  (interactive)
  (setq *cscope-use-tpath* (not *cscope-use-tpath*))
	(cscope-modify-mode-line)
  )

(defun cscope-toggle-trace-tpath ()
  "toggle usage of tpath in finding files"
  (interactive)
  (setq *cscope-trace-cstpath* (not *cscope-trace-cstpath*))
	(cscope-modify-mode-line)
  )

(defvar cscope-after-search-hook nil "hook to run after each successful search")

(if cscope-mode-map
    ()
  (setq cscope-mode-map (make-sparse-keymap))
  (define-key cscope-mode-map "n" 'next-line)
  (define-key cscope-mode-map "j" 'next-line)
  (define-key cscope-mode-map " " 'next-line)
  (define-key cscope-mode-map "p" 'previous-line)
  (define-key cscope-mode-map "k" 'previous-line)
  (define-key cscope-mode-map "q" 'kill-this-scope)
  (define-key cscope-mode-map "\177" 'previous-line)
  (define-key cscope-mode-map "e" 'cscope-goto-from-list)
  (define-key cscope-mode-map "" 'cscope-goto-from-list)
  (define-key cscope-mode-map "o" 'cscope-goto-from-list-other-window)
  (define-key cscope-mode-map "v" 'cscope-view-from-list)
  (define-key cscope-mode-map "t" 'cscope-toggle-tpath)
  (define-key cscope-mode-map "d" 'cscope-toggle-trace-tpath)
  (define-key cscope-mode-map "`" 'next-cscope-hit)
  (define-key cscope-mode-map "?" 'cscope-show-from-list)

  (define-key cscope-mode-map "1" '(lambda () (interactive) (cscope-goto-getnum (list 1) )))
  (define-key cscope-mode-map "2" '(lambda () (interactive) (cscope-goto-getnum (list 2) )))
  (define-key cscope-mode-map "3" '(lambda () (interactive) (cscope-goto-getnum (list 3) )))
  (define-key cscope-mode-map "4" '(lambda () (interactive) (cscope-goto-getnum (list 4) )))
  (define-key cscope-mode-map "5" '(lambda () (interactive) (cscope-goto-getnum (list 5) )))
  (define-key cscope-mode-map "6" '(lambda () (interactive) (cscope-goto-getnum (list 6) )))
  (define-key cscope-mode-map "7" '(lambda () (interactive) (cscope-goto-getnum (list 7) )))
  (define-key cscope-mode-map "8" '(lambda () (interactive) (cscope-goto-getnum (list 8) )))
  (define-key cscope-mode-map "9" '(lambda () (interactive) (cscope-goto-getnum (list 9) )))
  (define-key cscope-mode-map "0" '(lambda () (interactive) (cscope-goto-getnum (list 0) )))

  )

(run-hooks 'cscope-init-hook)

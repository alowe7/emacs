(put 'post-people 'rcsid
 "$Id: post-people.el 1072 2012-08-15 18:44:36Z alowe $")

(require 'directories)

; this just needs to be more context sensitive...maybe integrate with tw...

; this takes the most current file matching the 
(defvar *ows-contact-file* 
  (let*  ((contact-directory "/work/overwatch/people")
	  (contact-file-regexp  "overwatch-phone-list-[0-9]*\.csv$")
	  (contact-file 
	   (when (file-directory-p  contact-directory)
	     (expand-file-name
	      (car (sort
    ...
    (unless (and contact-file (file-exists-p contact-file))   (warn (format "%s not found in %s"  contact-file-regexp contact-directory)))
    contact-file
    )
  )
